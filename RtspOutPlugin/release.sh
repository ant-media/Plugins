...
if [ -d /nix/store ]; then
	...
	# argLine is jacoco's, so it moves aside and is kept. Setting it flat leaves coverage empty.
	set -- -Djacoco.propertyName=jacocoArgLine "-DargLine=@{jacocoArgLine} -Djavacpp.pathsFirst=true"
fi
