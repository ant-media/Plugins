...
// ─── Error: no broadcast name ────────────────────────────────────────────────
if (!broadcastName) {
  showError(
    "No broadcast name provided. Add <code>?name=your-broadcast</code> (or <code>?broadcast=</code> / <code>?id=</code>) to the URL."
  );
  infoStatus.innerHTML = badge("no stream name", "red");
}
