...
#define ZIXI_ERROR_WRONG_SRP			-25
...
typedef void(*ZIXI_RTMP_HANDLE_WRITE_DATA_EXTERNAL)(int size, int error, void* user_data);
typedef void(*ZIXI_RTMP_WRITE_DATA_EXTERNAL)(unsigned char* data, int size, void* param, ZIXI_RTMP_HANDLE_WRITE_DATA_EXTERNAL callback, void* user_data);

typedef void(*ZIXI_RTMP_HANDLE_READ_DATA_EXTERNAL)(int size, int error, void* user_data);
typedef void(*ZIXI_RTMP_READ_DATA_EXTERNAL)(unsigned char* data, int size, void* param, ZIXI_RTMP_HANDLE_READ_DATA_EXTERNAL callback, void* user_data);
...
typedef enum 
{
	ZIXI_PROTOCOL_UDP,
	ZIXI_PROTOCOL_MMT,
	ZIXI_PROTOCOL_RIST_SIMPLE,
	ZIXI_PROTOCOL_RIST_MAIN,
	ZIXI_PROTOCOL_HTTP,
	ZIXI_PROTOCOL_HTTPS
} ZIXI_PROTOCOL;
...
typedef enum
{
	...
	ZIXI_FAILOVER_MODE_RTP,			//	merge by RTP headers. SMPTE 2022-7
	ZIXI_FAILOVER_MODE_CONTENT		//	merge by RTP headers if available or by TS content.
} ZIXI_FAILOVER_MODE;
