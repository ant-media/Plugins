...
@param		 arq_mode			- one of ZIXI_ARQ_MODE values (default ZIXI_ARQ_ON)
...
@param		dtls_ciphers			- list of ciphers for DTLS handshake, in OpenSSL format (colon seperated). null for default.
									- Valid ciphers list:
...
@return		ZIXI_ERROR_OK
*/
int DLL_EXPORT zixi_connect_url(void *handle, const char* url, bool low_latency, ZIXI_CALLBACKS callbacks, bool ignore_dtls_cert_error, bool dtls_only, bool skip_rtp, const char* dtls_ciphers);
...
@return		ZIXI_ERROR_OK
*/
int DLL_EXPORT zixi_async_connect(void *handle, const char* host, unsigned short port, int index, bool low_latency, ZIXI_CALLBACKS callbacks, bool ignore_dtls_cert_error);
