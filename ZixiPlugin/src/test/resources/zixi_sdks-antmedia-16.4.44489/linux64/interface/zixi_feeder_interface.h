...
typedef struct 
{
	...
	int						fec_block_ms;			//	Maximum time of FEC block. default=30ms
	...
	bool					must_use_dtls;			//	Connect only if the connection is using DTLS encryption
	...
	bool					allow_arq;				//	Allow retransmissions on UDP based protocols. default=true

	ZIXI_PROTOCOL			protocol;				//	protocol type: udp/mmt/rist/http/https. When using http/https - only the first host and port parameters are used, and the rest of the paremteters are ignored.

	char* sz_dtls_ciphers;							//	Custom ciphers for DTLS handshake, in OpenSSL format (colon seperated). null for default.
													//	Valid ciphers list:
...
typedef struct
{
	...
	bool learn;									// learn channel to improve rate control over time
...
typedef struct _zixi_rtmp_out_config
{
	...
	ZIXI_RTMP_WRITE_DATA_EXTERNAL	external_write;
	ZIXI_RTMP_READ_DATA_EXTERNAL	external_read;
	void*				user_param;
/*
...
typedef struct
{
	...
	bool					ignore_dtls_cert_error;	// Ignore certificate error on DTLS connection. Use with caution, only when server is trusted.
	bool					must_use_dtls;			// Use only DTLS. When false - automatically detect if DTLS can be used.
	char*					sz_dtls_ciphers;		// Custom ciphers for DTLS handshake, in OpenSSL format. null for default.				
} zixi_upload_config;
...
@return		ZIXI_ERROR_INVALID_PARAMETER
*/
DLL_EXPORT int zixi_get_socket(void* stream_handle, int* socket);
