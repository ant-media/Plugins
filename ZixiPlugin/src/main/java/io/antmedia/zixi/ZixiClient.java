...
/**
 * ZixiClient connects to a Zixi Broadcaster(ZB) and pull the stream. It's also
 * called Zixi Receiver because
 * it receives the data from the Zixi Broadcaster
 * 
 * Zixi Receiver has two modes. Pulling the stream from ZB and
 * accepting/ingesting the stream pushed by Zixi Feeder or ZB
 * AMS supports only pulling the stream from ZB
 * 
 * In other words, ZixiClient is a StreamFetcher for the Ant Media Server
 * terminology
 * 
 * @author mekya
 *
...
public class ZixiClient {
	 ...
	 * 
	 * @Override
	 * public void call(org.bytedeco.javacpp.Pointer userData, int level,
	...
	public ZIXI_NEW_STREAM_FUNC newStreamFunction = new ZIXI_NEW_STREAM_FUNC() {
		...
		public void call(org.bytedeco.javacpp.Pointer zixi_handler, org.bytedeco.zixi.client.ZIXI_STREAM_INFO info,
				org.bytedeco.javacpp.Pointer user_data) {
	...
	public ZIXI_BITRATE_CHANGED_FUNC bitrateChangedFunction = new ZIXI_BITRATE_CHANGED_FUNC() {
		...
		public void call(org.bytedeco.javacpp.Pointer zixi_handler, int stream_index, int bitrate,
				BytePointer stream_name, org.bytedeco.javacpp.Pointer user_data) {
	...
	public ZIXI_STATUS_FUNC statusChangedFunction = new ZIXI_STATUS_FUNC() {
		...
		public void call(org.bytedeco.javacpp.Pointer zixi_handler, int status,
				org.bytedeco.javacpp.Pointer user_data) {
			...
			try {
				...
				while ((packet = zixiClient.getQueue().poll()) == null) {
					if (zixiClient.stopRequested.get()) {
						logger.info("Stop request for stream id: {} and scope: {}", zixiClient.streamId,
								zixiClient.appAdaptor.getScope().getName());
						break;
					}
					...
					if (waitCount % 50 == 0) {
						logger.warn("Stream:{} in {} does not get packet for {} ms", zixiClient.streamId,
								zixiClient.appAdaptor.getScope().getName(), elapsedTimeMs);
					}

					if (elapsedTimeMs > INACTIVITY_TIMEOUT_MS) {
						logger.warn("Inactivity timeout({}ms) for srt stream:{}/{}", INACTIVITY_TIMEOUT_MS,
								zixiClient.appAdaptor.getScope().getName(), zixiClient.streamId);
						zixiClient.stop();
						break;
					}
				}
	...
	public boolean connect() {
		...
		ret = zixi_configure_error_correction(zixiHandle, latency, ZIXI_LATENCY_STATIC,
				fec ? ZIXI_FEC_ON : ZIXI_FEC_OFF, fec_overhead, fec_block_ms, fec_content_aware, false, 0, false);
		if (ret == 0) {
			...
			readTimer = vertx.setPeriodic(10, h -> {
				...
				do {
					...
					if (lastReceivedPacketTimeMs != -1 &&
						...
						logger.warn("No activity more than {} for Zixi Client with streamId:{} so stopping connection",
								INACTIVITY_TIMEOUT_MS, streamId);
						stop();
						break;
					}
	...
	public Result stop() {
		...
		vertx.executeBlocking(b -> {
			...
			if (isPipeReaderJobRunning.compareAndSet(false, true)) {
				logger.info("while stopping, number of items in the queue:{} streamId:{} and scope:{}",
						getQueue().size(), streamId, appAdaptor.getScope().getName());
	...
	private synchronized void releasePrepareContext() {
		...
		if (inputFormatContext != null) {
			logger.info("Releasing input format context:{} for stream:{} and scope:{} ", inputFormatContext, streamId,
					appAdaptor.getScope().getName());
	...
	private void processPacket(boolean callItSelfIfRequired) {
		...
		if (isPipeReaderJobRunning.compareAndSet(false, true)) {
			if (inputFormatContext == null) {
				logger.warn("Zixi stream read is called after it's disconnected for stream:{} and scope:{}", streamId,
						appAdaptor.getScope().getName());
			} else {
				if (av_read_frame(inputFormatContext, pkt) >= 0) {
					...
					logger.info("Cannot read packet for stream:{} and scope:{}", streamId,
							appAdaptor.getScope().getName());
				}
			...
			logger.trace("By passing the processpacket because there is already a thread running inside for stream:{}",
					streamId);
		}

		if (callItSelfIfRequired && !getQueue().isEmpty() && !stopped.get() && inputFormatContext != null) {
			// call execute process packet if not empty and not stopped and not
			// inputFormatContext null
